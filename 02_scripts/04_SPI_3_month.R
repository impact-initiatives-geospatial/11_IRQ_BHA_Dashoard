...
spi_image_collection <- ee_chirps_spi(chirps_tidy,window = 3,window_unit = "month",moi = c(1:8))
...
spi_image_collection_processed <- spi_image_collection %>% filter(
  year > previous_year
) %>% select("precipitation_sum_3_month_SPI")
...
write.csv(spi_bind,"05_outputs/01_precipitation_defict/three_month_spi.csv")
